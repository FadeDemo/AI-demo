...
除非单课另有说明，本专题课程任务产生的可运行实现都在 [LLM Terminal Assistant](../../projects/llm-terminal-assistant/README.md)中按课程顺序增量扩展，不为每一课另建功能重复的项目。代码、测试、配置、运行命令和项目行为说明保存在该项目中；调用链、实验数据、观察结论和知识检查等书面记录统一保存在[练习回答目录](answers/index.md)中，不在回答文档中重复代码。

每一课的任务正文会说明该课需要修改的项目能力和应提交的书面记录。回答文档只用于留存学习结果，不能代替课程中的任务说明和通过条件。

首次完成某课的书面任务时，才在 `answers/` 下创建与课程正文同名的回答文件，例如 `tokens-and-context.md` 对应 `answers/tokens-and-context.md`；文件实际创建后再把它加入回答目录，不预先创建空白答题模板。

## 配套交互实验

[LLM 使用基础交互实验](../../projects/llm-usage-lab/index.html)不访问网络，也不调用真实模型。它用可调参数演示两件容易混淆的事：

- 在单次模型调用中，本轮活跃输入与本轮输出预算共同受到上下文窗口上限约束，不能把本轮用户输入的 Token 数当作整次调用的总占用。
