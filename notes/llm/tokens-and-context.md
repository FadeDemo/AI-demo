...
完成本课后，应用已经能判断一次请求是否装得下，并为回答预留输出空间。但“允许模型生成多少”还没有回答“模型会怎样从候选内容中生成这段输出”：为何同一请求可能得到不同措辞，为什么要求稳定结果的固定字段抽取与要求多样候选的标题构思不一定采用相同的生成策略，以及怎样识别回答只是达到输出上限而被截断。下一课[生成参数与采样](generation-parameters.md)将处理这些问题。
