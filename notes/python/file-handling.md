...
- 防范路径遍历攻击：用户传入的文件名可能包含 `../` 等相对路径组件。直接把用户输入拼到 `output_dir / user_filename` 后，应调用 `.resolve()` 解析路径，再确认它仍在允许目录内，否则拒绝操作。
- 不要加载来源不可信的 `pickle` 文件。`pickle` 是 Python 用来序列化对象的模块（把 Python 对象存成文件再还原），但还原过程可以执行任意代码，因此只应 `pickle.load()` 你完全信任的数据。
