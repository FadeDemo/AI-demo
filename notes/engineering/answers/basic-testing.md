...
| 测试层次   | 验证范围                                     | 本项目中的对应情况                                                     |
| ---------- | -------------------------------------------- | ---------------------------------------------------------------------- |
| 单元测试   | 隔离验证一个函数、类或小模块                 | 直接调用 `search_documents()`，检查返回值、异常和日志字段              |
| 集成测试   | 验证多个产品组件或产品代码与真实边界协同工作 | 如果增加 Markdown 加载模块，可验证加载器产生的 `Document` 能被正确搜索 |
| 端到端测试 | 从用户可见入口运行完整流程并检查最终结果     | 从命令行启动 `engineering-demo`，检查退出状态、stdout 和 stderr        |

配套项目的核心搜索逻辑接收内存中的 `Document` 列表，不连接数据库、网络、模型 API 或产品级文件加载器。大部分测试直接调用 `search_documents()` 并观察返回值、异常和日志，因此能快速、确定地定位搜索规则中的问题，主要属于单元测试。当前 pytest 套件没有从已安装的命令行入口启动完整应用，所以不能把这些函数级测试称为端到端测试。
...
集成测试仍可以直接调用若干产品组件，不必经过用户入口；端到端测试则应从命令行或其他公开入口覆盖完整链路。三者不是按价值高低排列：单元测试适合快速覆盖规则，集成测试发现组件连接问题，端到端测试确认用户实际经过的关键路径。当前内存项目的主要风险集中在搜索、校验和日志契约，所以以单元测试为主是合适的；如果以后接入真实文件、数据库或远程服务，就需要相应增加集成测试，并为少量关键流程保留端到端测试。
